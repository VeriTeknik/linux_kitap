...
![](../.gitbook/assets/Ken\_n\_dennis.jpg)

_Ken Thompson (solda) ve Dennis Ritchie (sağda)_
...
C programlama dili, Dennis Ritchi tarafından Unix işletim sistemi için geliştirilmiştir. Unix işletim sisteminin büyük çoğunluğu C ile yazılmıştır. C programlama dili, hem pek çok donanım üzerinde çalışabildiğinden, hem de öncülleri BCPL veya B gibi tip kullanmayan (typeless) bir dil olmadığından, ideal bir _sistem programlama dili_ olarak nitelendirilir.

![](../.gitbook/assets/c.jpg)

C programlama dilinin yaygınlaşmasına yardımcı olması dışında, bilgisayar bilimlerine yaptığı katkılardan ötürü Brian Kernighan ve Dennis Ritchie'nin yazdığı _The C Programming Language_ kitabının yayımlanması da bir dönüm noktası kabul edilir.
